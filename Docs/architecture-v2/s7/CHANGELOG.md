...
- **已连接设备并入连接列表并置顶（#1）**：S7 手表**连接后常停止广播**，从扫描结果消失 → 连接页看不到已连设备。两连接页(`ConsoleConnectViewModel` / `DeviceScanViewModel`)把 `registry.connected` **并回**扫描结果(不在结果里的补入)，并让已连设备**豁免无名/RSSI 过滤**始终可见；排序仍「已连接置顶」。真机确认：连上的 S7-FCC4 即便停广播也钉在列表首位显示「已连接」。
- **控制台头卡就地断开（#2）**：`DISCONNECTED` 有「↻ 重连」；连接态补对称的 **「⛓ 断开」红按钮**(`disconnect()` → `ble.disconnect` + `registry.remove`)。设备仍**黏附**控制台(链路收集器改为已选设备始终保持)，断开后就地显示 `重连`，可来回 `断开 ↔ 重连`，不必回连接页。断开后手表恢复广播、在连接页重新可见为「未连接」。
- **真机验证**（Redmi / Android 13 + S7-FCC4）：连接页已连设备置顶显示「已连接」；控制台连接态显示红「断开」→ 点它就地断开变蓝「重连」+ 指令区置灰 → 点「重连」回到「断开」并自动重刷设备信息(电压 4263→4264 mV)。截图 `assets/q_*.png`
