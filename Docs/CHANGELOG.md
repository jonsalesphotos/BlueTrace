...
## [UWTP] v0.2-draft 客户端首版：离线文件上传（手表→App）可联调 — 🚧 2026-07-15（分支 feat/uwtp-v02-client，真机 NOT_RUN）
- **协议真源**：apollo4_watch_s7 仓 `S7_UWTP_v0.2_工作稿.md`（2026-07-15 实质审查 D-1~D-10 修订版）+ `proto_v02/` DRAFT（.proto/.options/18 个 golden，逐字节快照入 [`shared/proto/uwtp_v02/`](../shared/proto/uwtp_v02/README.md)）。范围 = P2 首交付链路：CTRL INFO/STATE_QUERY + TRANSFER OBJECT_LIST/BEGIN/DATA/ACK/FINISH/ABORT/DELETE/ERROR_EVENT；LIVE 仅消息编解码（会话控制未接）。
- **shared/uwtp 新包（KMP commonMain，零新依赖）**：[`UwtpFrame.kt`](../shared/src/commonMain/kotlin/io/bluetrace/shared/uwtp/UwtpFrame.kt) 5B Core Header 编解码（0x14 命令/0x10 数据响应帧；EXT_HDR/SEG 非零/len 不符/角色 1+1 按 §5 丢弃计数）；[`Pb.kt`](../shared/src/commonMain/kotlin/io/bluetrace/shared/uwtp/Pb.kt) **手解 proto3 wire 子集**（varint/fixed64/LEN；默认值省略 + 未知字段跳过 + 截断拒绝 = §6.3/§6.8 契约，不给 KMP 工程引 protobuf 工具链）；[`UwtpMessages.kt`](../shared/src/commonMain/kotlin/io/bluetrace/shared/uwtp/UwtpMessages.kt) 全消息 + TRANSFER_DATA 二进制前缀（transfer_id+offset[+frame_crc32] LE）+ CRC32 参考实现；[`UwtpClient.kt`](../shared/src/commonMain/kotlin/io/bluetrace/shared/uwtp/UwtpClient.kt) **同管道分流**（首字节 0xBB=B2A 不碰、0x1X=UWTP，B2A 全路径零改动）+ App-owned 单飞命令面（seq 配对/迟到响应丢弃/错误码异常化）+ DATA/EVENT 路由；[`UwtpFilePull.kt`](../shared/src/commonMain/kotlin/io/bluetrace/shared/uwtp/UwtpFilePull.kt) 传输状态机（accepted_* 生效/offset 幂等去重 + 范围归并补洞/**重复帧也计窗口帧数**防 ACK 丢失僵持/累计 ACK 每 accepted_ack_every_n 帧或到尾/旧 transfer_id 帧与事件丢弃计数/ERROR_EVENT 断点回退 last_valid_offset/一切退出路径截 .part 到连续位）+ 实时吞吐（2s 滑窗 KiB/s + 会话平均）。
- **断连续传（D-7 连接域模型）**：进度记 App 侧——[`UwtpPullStore`](../app/src/main/java/io/bluetrace/data/android/UwtpPullStore.kt) 持久化 `{fileId, objectToken(BEGIN_RSP 权威值), totalSize, contiguous}`（每次 ACK 更新）+ `.part` 长度恒等于断点；重连后凭 contiguous+token 重新 BEGIN；**OBJECT_CHANGED（DRAFT 暂用 0x1101）→ 清断点自动从 0 重来一次**。
- **DEBUG「UWTP 传输」屏**（设置页 DEBUG 区，与 OTA 固件同级）：连接（BLE 层既有 requestMtu(247)）→ 查能力 INFO/查状态 STATE → 列文件（含断点/已保存标注）→ 拉取/续传（进度条 + 实时/平均 KiB/s + 帧/重复/旧id/CRC错计数）→ 中止/清断点 → 完成件落 `files/uwtp/<MAC>/file_<id>.col2`。接线四处最小化：Routes/BlueTraceApp/SettingsScreens/AppModule。
- **测试 53 例全绿（shared 总 258 + app 5 零回归）**：18 个 golden 逐字节对拍（请求编码==golden；响应解码断言+重编码==golden；DATA 前缀双向）；PB 语义（乱序/未知字段/`08 00`≡省略/截断与坏 wire 拒绝/uint32 截断）；帧层（坏帧六型）；状态机 14 例（去重/补洞/纯重发窗口 ACK/stale id 与 stale 事件/ERROR_EVENT 断点回退/中止/断连/续传绝对 ACK/BUSY 保断点/OBJECT_CHANGED 弃断点/crc_mode=1 坏帧/空闲超时）；客户端分流 5 例。**坑存照**：Kotlin 块注释嵌套——注释里写 `目录/*.proto` 会把 `/*` 当嵌套注释开口吞掉整个文件（三处中招）。
- **遗留**：真机 NOT_RUN（固件侧 collect_link 实现中，联调后按 P0 实测回填 ack_every_n/T_ack/吞吐）；DELETE 有客户端 API 无 UI；LIVE 会话控制与 RAW_CHUNK 解码待固件裸流可用后接（P4）；frame_crc32 覆盖式样待专项⑤（当前按 transfer_id||offset||data 参考实现，阶段一固件恒回 crc_mode=0 不触发）。
...
- **分层观测 fork 定位真凶层**：fork 官方 tag beta03（本体已归档 [`真机证据/nordic25_20260716/nordic_observation.patch`](真机证据/nordic25_20260716/nordic_observation.patch)，与 Maven sources 逐字一致仅换行符差异；3 处只读日志 `DISC_REQUEST/DISC_CALLBACK/DISC_CONSUMED`；复合构建 `-Pbluetrace.nordicFork=true` 门控接入，默认关）。四层判决：`started=false` **0 次**（Framework 从未拒绝）；**丢失发生在 `gatt.discoverServices()` 返回 true 与原生回调之间，且只在 Nordic 侧**；`subs=0`/`tryEmit=false` **0 次**（SharedFlow(replay=0) 丢事件假设**证伪**，CALLBACK 与 CONSUMED 恒等）；App 消费清白。
- **A/B 判决（同机同表同栈，中间不重启蓝牙；hold=连接后 12s 不取消）**：自写 8 次请求 **0 挂死** vs Nordic 7 次 **3 挂死（43%）**，唯一 cancel 晚于全部挂死。时序相关性：3/3 挂死均为 **MTU 协商落在发现在飞期间**（健康时 CB 先于 MTU 3/3）——与库自身注释"并发 GATT 操作会丢回调"呼应；自写只在 `onMtuChanged` 里发现（严格串行）故免疫。**只报相关性未证机制**（两者都过 OperationMutex，MTU 标记是 App 侧观测点）。
- **上游 issue 已提交**：[NordicSemiconductor/Kotlin-BLE-Library#337](https://github.com/nordicsemi/Kotlin-BLE-Library/issues/337)（英文，含两缺陷源码定位/A/B 表/三层数据/可折叠观测补丁/5 条修复建议；明确声明 MTU 相关性未证机制）。底稿 [`设计/Nordic重连挂死_issue草稿.md`](设计/Nordic重连挂死_issue草稿.md)；根因分析补终局后记 [`设计/Nordic重连挂死_根因分析.{md,html}`](设计/Nordic重连挂死_根因分析.md)。
- **孤儿连接（自家架构病，两 agent 全量审计 + 真机三方取证坐实）**：9 处连接入口**全部**跑 `viewModelScope`（4 处从不 `registry.add`，其余 connect 与 add 是两条独立语句）——退屏腰斩 connect 后**物理连接无人持有**：设备连着即停广播 → 连接/扫描/OTA/控制台/采集**五页同时看不见**（全部以 `registry.connected` 为唯一真相源）；`disconnect(id)` 因 conns 已无该 id 而 `?.let` 短路（一字节不发）却仍**谎报 DISCONNECTED**；全仓无 conns↔registry 对账；孤儿在库对象层永远可达（`managedPeripherals` 按 MAC 缓存永不清除）但 API 层完全不可达（枚举 protected/forceClose internal/disconnect 首行撞锁），**回收杠杆只有"代"粒度**。Nordic 晚登记（订阅完成才入表，30s 无句柄窗口）是主产地；**自写早登记 + 取消时同步 `gatt.disconnect()+close()` 天然免疫**。串口 MCP（SuperConnectX/COM4）首次接入取证：表侧 `conn/adv` 直读，"连着即停广播"实锤。
- **修法 = [`BleConnectionCoordinator`](../shared/src/commonMain/kotlin/io/bluetrace/shared/ble/BleConnectionCoordinator.kt)（app 级连接事务宿主，后端无关，提交 1 已合 main）**：连接=意图提交立即返回，事务跑 app 域；**页面销毁不取消事务**；`ble.connect → 确认 CONNECTED → registry.add` 同一事务；只有显式 disconnect 才取消并接管完整清理；**底层断开真正返回前不发布 Idle**；token CAS 争槽（与 OTA 侧 Lease/RunToken 同源），每 token 最多善后一次。8 例测试逐条对应复核验收项（含"VM 取消等待不取消 app 级事务""旧清理不碰新事务"）。**提交 2（接线两个扫描→连接页）随后**。
- **仓库卫生**：三条已验收分支（#24A/#27 正名/Coordinator）合入 main 并推送（`cbe3aa1→bab7e76`，合并后 216/33 类与基线逐条相同证零行为改动）；本地只剩 `main` + `feat/uwtp-v02-client`（后者已 rebase 到最新 main，280/0 全绿，pre-rebase 锚 tag `uwtp-v02-prerebase`）；`E:\BlueTrace-uwtp` worktree 与 07-09 旧 worktree 均清除；`task/25` 推远端归档后删本地。**坑**：`.gitignore` 的 `*.log` 曾把三份真机证据日志**静默挡在库外**（`git add` 忽略无告警，提交信息"证据已入库"实为误报）——本次 `git add -f` 补入 [`真机证据/nordic25_2026071{5,6}/`](真机证据/nordic25_20260716/)；`NordicOperationMutexPinTest`（库行为跳闸丝，升 Nordic 版本时若失败=上游已改行为）一并从 task/25 捞回 main。**07-16 补归档**：观测 fork 的 3 文件 diff（+38/-4，基线官方 tag beta03 = `78843bf`）存为 [`真机证据/nordic25_20260716/nordic_observation.patch`](真机证据/nordic25_20260716/nordic_observation.patch)（SHA-256 记录在同目录 README；删除源目录前经 `git apply --check -R` 验证与工作区逐字节一致），`E:\_nordic_fork` 目录随之删除——至此 #25 全部独有产物（日志+补丁+实验接线）均已入库或封存 `origin/task/25-nordic-reconnect-hang`。

## [架构·命名] #27 一阶段：协议正名 S7 → B2A（纯搬迁，零逻辑）+ 设计 v2 按用户纠正重写 — ✅ 2026-07-15
**用户纠正（架构级，v1 设计据此作废重写）**：**"S7" 是设备型号（产品名，会改），"B2A" 才是协议名；GATT UUID 才是稳定锚点。批量 OTA 是软件功能，协议是单独注入的，必须分离。** v1 设计里"把批量控制器改名 `S7MultiOtaController` 并固化为 S7 专属工具"的建议**方向是反的、已撤回**——那等于把一个软件功能反向绑死到设备型号，是制造架构债而非固化边界。**正确分层**：GATT UUID 只负责识别 → Catalog 识别一次并**注入能力对象**（`FirmwareUpdateCapability` = 包解析器 + 升级策略 + 中止语义 + 回连/验证语义）→ **通用 OTA 软件编排器**（队列/重试/Gate-Lease/停止善后/逐任务调用注入的能力）**零协议分支、零产品名、零 UUID**。
- **一阶段落码 = 纯搬迁，零逻辑改动**（`git mv` 保留历史，49 文件）：包 `io.bluetrace.shared.s7` → `io.bluetrace.shared.b2a`（commonMain + commonTest）；`S7Xxx` → `B2aXxx`（Console/DeviceProfile/DeviceControl/VendorOps/Frame/FrameCodec/Crc/Messages/FileTrans/OtaSession/FirmwareUpdateStrategy+Factory/MockWatch 等 **27 个类型**）；`object S7` → `object B2a`；`PROFILE_S7` → `PROFILE_B2A`；`S7_TEST_MAC` → **`TEST_DUT_MAC`**（它指一台物理测试设备而非协议，故取中性角色名）；`isS7`→`isB2a`。FFE0/FFE1/FFE2 继续作为 GATT 识别与通道声明。
- **严格保护未动**（改了即行为/事实变化）：`"SKG.S7.B2A"` = `PROFILE_B2A` 的**值**（写进 manifest 的运行时数据）；`"SKG WATCH S7-FCC4"` 等 = **真实广播名**（设备事实）；`"s7-fcc4"` 等 = Mock roster 设备 id。搬迁用 `\bS7[A-Z]` 模式，**天然避开**上述三类（其 S7 后面跟的是 `-`/`.`/空格而非大写字母）。
- **顺带勘误（零逻辑）**：[`Device.kt`](../shared/src/commonMain/kotlin/io/bluetrace/shared/domain/Device.kt) 原 KDoc 称"正式识别按广播名前缀 `SKG WATCH S7-`"——**与实际代码矛盾**（`B2aDetect.matchesAdvertisement` 按广播 **FFE0** 判定，且其自身 KDoc 早已写明"名称前缀仅作展示/过滤辅助，不再是协议判据"），且指向已迁走的 `Docs/归档/s7`。改写为："profileId 是识别**结果**的稳定标识而非判据；识别锚点是 GATT UUID；产品名只用于 UI 展示"——正是用户所指"看起来像要换协议实现"的那类误导债。
- **零行为改动的证明**：`shared 216/0 + app 9/0 + 33 个测试类`，与基线 `cbe3aa1` **逐条相同**；assembleDebug 通过；一次编译即过、零 S7 标识符残留。
- **遗留待决**：`PROFILE_B2A` 的值仍是 `"SKG.S7.B2A"`（含厂商+产品名）。对照既有惯例 `PROFILE_HRS = "HeartRate.SIG.0x180D"`（协议.组织.UUID），它应为 `"B2A.SKG.0xFFE0"`；但**改值 = 改写进 manifest 的数据**，不属"纯搬迁"，单独立项（倾向改，作独立可回退提交）。
- **二~四阶段设计**（[`设计/通用OTA分派_设计.{md,html}`](设计/通用OTA分派_设计.md) v2，1 图）：二=批量编排与协议分离（`MultiOtaController` 移出 b2a 包进通用 OTA 包，入队时保存 `FirmwareUpdateJob(device, capability, pkg)`，执行只调 `job.capability.createStrategy(...).run(job.pkg)`；**租约/RunToken 所有权模型是协议中立的软件资产，整体留通用层**；含 D-1 签名净化——`reconnectScanMs`（"自复位回连"的 B2A 私有假设，ZX 工厂明说未使用）下沉协议私有，`abortScope` 保留）；三=预检走 `DeviceControl` 六分面（`control.battery`/`control.info`，分面 null 即跳过；**低电阈值是软件策略、数据来源是通用分面**）；四=包解析注入（`:app` 只解容器 `Uri/zip/bin → List<RawFwEntry>`、协议侧只解语义 `RawFwEntry → FwPackage`，`OtaPackageValidator` 整段搬进 B2A loader 逻辑零改动；ZX 顺带补真 loader——现在 `ZxPackage()` 只在测试里手搓）。
- **新验收标准**：① 改广播名、只要仍广播 FFE0 即识别为同一协议；② 通用 UI/VM/批量控制器**无 S7/B2a/FFE0 分支**；③ **两个 fake 协议跑同一批量控制器**各调各自策略；④ ZX 链路**零 B2A/FILE_TRANS 帧**（帧捕获）；⑤ B2A 行为与四条真机红线零变化；⑥ 一批任务限同一 protocolId + 兼容包——**包兼容约束，不是设备名称约束**，结构仍支持将来混合协议队列。
- **已知结构级假设待处理**：编排器"升级后用同一 BLE id 复读电量"——对 B2A 成立（自复位回连同 id），对"升级后切 Bootloader 服务/换地址"的协议会读错设备；二阶段应由**策略返回升级后的设备标识**（或声明身份不变），而非编排器写死。

## [OTA·设计] ~~#27 通用 OTA 分派：耦合面测绘 + 分阶段设计稿~~ — 📐 2026-07-15〔⚠️ **#27B"正名为 S7MultiOtaController"的建议已被上一条推翻并撤回**；耦合面测绘部分仍有效〕
