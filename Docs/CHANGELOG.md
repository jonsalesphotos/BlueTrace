...
## [BLE·Nordic] #25 重连服务发现挂死：根因闭环（库缺陷坐实）+ 幽灵锁规避 — 🔬 2026-07-15（真机待验）
分支 `task/25-nordic-reconnect-hang`。开工前先封存稳定基线 tag [`baseline-20260715-abstraction-w6`](https://github.com/jonsalesphotos/BlueTrace/releases/tag/baseline-20260715-abstraction-w6) → `cbe3aa1`（强制重跑复核 shared 216/0 + app 9/0，非缓存）。
- **根因 = 库缺陷，两个 bug 叠在同一段三行代码**（beta03 官方 sources jar 逐行核对）：① **触发器**——库丢弃 `gatt.discoverServices()` 的 **false 返回值**（调用点 `client-core Peripheral.kt:510` 无任何 if；接口 `:201` 明确声明 Boolean；实现 `client-android NativeExecutor.kt:139` 在原生拒绝启动发现时返回 false），而库已先把状态置成 `Discovering`(:509) → 回调永不到达 → **状态永久停 Discovering**（=实测 10s 超时）；② **放大器**——该挂住的发现**持有全进程唯一的静态 Mutex 且永不释放**：`discoverServices` 是全库**唯一**用裸 `OperationMutex.lock(ServicesChanged)` 而非自带 finally 的 `withLock` 之处(:504)，三条解锁路径(:411 ServicesDiscovered / :466 ServiceDiscoveryFailed / :360 invalidateServices)**全部依赖事件到达**；库内其余一切 GATT 操作均走 `withLock(owner=null)` 排在其后 → **任何设备的任何 GATT 操作永久挂起**（="重试必挂""仅杀进程可解"）。一次普通发现失败被放大成全进程蓝牙永久瘫痪。
- **三个原假设的判决（两个证伪）**：全局单例 ✅ **坐实**（`object OperationMutex` + 单个 `private static final Mutex`，javap + 源码双证）；**"取消订阅的 CCC disable 写在全局锁上泄漏" ❌ 证伪**——写/读/订阅全走 `withLock`（= kotlinx `Mutex.withLock`，**自带 try/finally**），取消即释放，测试对照组坐实；**"serviceDiscoveryRequested/servicesDiscovered 闩不复位" ❌ 证伪**——`servicesDiscovered` 在 `invalidateServices` 复位(:357)，`serviceDiscoveryRequested` 是库**故意**不复位(:365-369 注释写明为重连留观察者)。**故 2026-07-15 上午按 CCC 写假设做的"断开换序"缓解方向错了——这正是"换序后仍复现"的原因**（换序本身无害，予以保留）；执行笔记旧条目已就地勘误并链接权威版本。
- **因果定序的硬证据**：真机日志中重连的 **MTU 协商成功（mtu=247）**，而 MTU 请求正走 `withLock`（`core-android Peripheral.kt:599`）→ 证明**那一刻锁是空闲的** → 顺序是"发现失败 → 制造幽灵锁 → 后续全挂"，而非"锁挡住了发现"。
- **owner 语义形同虚设**：owner token 是 `data object ServicesChanged`（`GattEvent.kt:88`，全进程唯一实例，所有 peripheral 共用）→ 第二台设备发现时 `lock` 因同 owner 抛 ISE，**库 catch 后不持锁继续执行发现**（串行保证失效）；且任一方可解他方的锁。
- **可执行证据 + 跳闸丝**：[`NordicOperationMutexPinTest`](../app/src/test/java/io/bluetrace/data/android/NordicOperationMutexPinTest.kt) —— app JVM 单测 3 例（**不需真机/蓝牙**，直接驱动库真实 `OperationMutex`）：锁是全进程一把 / 裸 lock 持有者被取消后锁仍被持有（对照组证明 withLock 取消即释放）/ 同 owner 二次 lock 抛 ISE 且任一方可解他方锁。将来升 Nordic 版本时本测试若失败 = 上游改了行为，届时评估撤掉本项目侧规避。
- **已落规避（只拆放大器，不修触发器）**：[`NordicBleClient.sweepGhostServiceDiscoveryLock`](../app/src/main/java/io/bluetrace/data/android/NordicBleClient.kt) —— teardown 后经 4s 宽限期（让库自身 `invalidateServices` 先有机会）仍 `holdsLock(ServicesChanged)` 即判定幽灵并强制 unlock，把"全进程永久瘫痪（仅开关蓝牙可解）"降级为"本次连接失败、可立即重试"。**实现要点**：清扫**独立成协程**——不可串在 `disconnectBounded` 之后，后者实证可能永不返回（结构化并发切不掉库内挂起），串行会让清扫永远等不到执行。**边界（已接受）**：并发发现场景会提前解掉他人的锁；本项目设备会话本就串行（DeviceSessionManager 单 Mutex、多设备 OTA 逐台）故不成立。
- **⏳ 真机未验 + 触发器未闭环**：原生层"为何主动断开后重连时 `discoverServices()` 返回 false"仍需三组对照（不取消订阅 / 物理断链后取消订阅 / 新建 Peripheral+CentralManager 回连）；候选=`disconnect()` 只调 `gatt.disconnect()` 不调 `gatt.close()`（`NativeExecutor.kt:266-274`，client interface 未释放；`close()` 才两者都调 :276-295），与"反复后 force-stop 首连也挂、需开关蓝牙"（系统栈自身脏，进程级锁无法解释）吻合。**验收**=同机主动断开→回连 ≥50 次、`Discovering` 挂死 0 次、无需开关蓝牙，且 OTA 复位回连红线不回归。**验收通过前 Nordic 不得转默认**（#24B 硬前置；上游 v2 仍标 Beta 亦支持此判断）。
- 文档：[`设计/Nordic重连挂死_根因分析.{md,html}`](设计/Nordic重连挂死_根因分析.md)（1 图：全局锁泄漏链）；上游 issue 草稿 [`设计/Nordic重连挂死_issue草稿.md`](设计/Nordic重连挂死_issue草稿.md)——**属对外发布动作，待用户过目后由用户自行提交**。
