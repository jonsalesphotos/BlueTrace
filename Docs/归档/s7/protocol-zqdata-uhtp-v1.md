...
> **状态**：设计稿 V1（2026-07-06），待与固件端评审冻结。机器可读契约：[`zqdata_uhtp_v1_draft.proto`](zqdata_uhtp_v1_draft.proto)。示例包全部由脚本实算（[`assets/gen_zqdata_uhtp_examples.py`](../../S7B2A/assets/gen_zqdata_uhtp_examples.py)），protobuf wire 与 CRC32 可复验。
> 现网协议（被替代对象）见 [`S7协议共识规格.md`](../../S7B2A/S7协议共识规格.md)：B2A TEST 0x10-0x12 + 40B→28B 重打包那套；迁移与共存见 §11。深度参考：同目录 [`protocol-b2a.md`](../../S7B2A/protocol-b2a.md)（B2A 全命令集）、[`protocol-zqdata.md`](../../S7B2A/protocol-zqdata.md)（现网 zqdata 全量审计）。
...
| 现网问题（见 [S7协议共识规格.md](../../S7B2A/S7协议共识规格.md)） | 本设计的解法 |
| --- | --- |
...
| --- | --- | --- |
| `0x01` | ECG1_V2 | 现网 `ecg_raw.dat`（32B 头 + 8B 帧头流 + ACC 段，小端；见 [S7协议共识规格.md](../../S7B2A/S7协议共识规格.md) §6） |
...
> 全部由 [`assets/gen_zqdata_uhtp_examples.py`](../../S7B2A/assets/gen_zqdata_uhtp_examples.py) 实算（protobuf wire + CRC32），len 自洽断言通过。示例 UTC = 1783324800（2026-07-06 08:00:00Z）。
...
- 被替代的现网协议：[`S7协议共识规格.md`](../../S7B2A/S7协议共识规格.md)（B2A + DC 0x10-0x12 + 40B→28B 重打包）；全量审计：[`protocol-b2a.md`](../../S7B2A/protocol-b2a.md)、[`protocol-zqdata.md`](../../S7B2A/protocol-zqdata.md)
- 机器可读契约：[`zqdata_uhtp_v1_draft.proto`](zqdata_uhtp_v1_draft.proto)（本文所有 Protobuf 消息的权威定义）
- 示例生成脚本：[`assets/gen_zqdata_uhtp_examples.py`](../../S7B2A/assets/gen_zqdata_uhtp_examples.py)
