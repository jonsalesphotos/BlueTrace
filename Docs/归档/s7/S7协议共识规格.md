...
>
> **与 BlueTrace v0 协议的区隔**：本文是 **S7 真机现行协议**（0xBB 信封 + Cmd/Key）；[`architecture/BLE协议帧规格_开发者版.md`](../自研协议线_v0/BLE协议帧规格_开发者版.md) 是自研 DUT 的 12B 帧头 + protobuf **草案**，两者互不相关，App 侧按通道分别挂解析器。
>
...
- BlueTrace 侧接入架构：分支 `02_parser_registry_design.md`（可注册通道解析器）；自研 DUT 草案（勿混淆）：[`architecture/BLE协议帧规格_开发者版.md`](../自研协议线_v0/BLE协议帧规格_开发者版.md)。
