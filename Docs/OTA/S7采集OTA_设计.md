...
> **真源**：现网 B2A 协议 = [`../归档/s7/S7协议共识规格.md`](../归档/s7/S7协议共识规格.md)；本文 FILE_TRANS 字段由固件 `apollo4_watch_s7` 反推（file:line 在册），**字节级偏移/字节序待真机抓包最终验证**（全仓唯一 golden 帧是产测握手，OTA 各帧无真机报文）。
...
| 子命令 | 方向 | payload |
|---|---|---|
| `REQ 0x04` | App→表 | `[0]moduleId=1 [1]ucIsOffset [2]fileCount [3]rsv [4-7]totalSize(LE)`；**fileCount/totalSize = 整场合计**（`:3685-3695`）。设备查磁盘(`totalSize/1024` vs 剩余 KB)，**OK 路等 MMI 授权后异步回 12B**：`status/moduleId/fileCount/currFileIdx/sliceMaxSize(LE)/offset(LE)`；`sliceMaxSize=(协商MTU−12)×17`。 |
...
- 现网 B2A 协议：[`../归档/s7/S7协议共识规格.md`](../归档/s7/S7协议共识规格.md)、[`../归档/s7/protocol-b2a.md`](../归档/s7/protocol-b2a.md) §9。
