...
>
> **状态**：v0.1 草案，**待与固件端共同冻结**（待冻结项见 SPEC §10.4 / [归档协议文档 §15](../legacy/BlueTrace_Protocol.md)）。文中示例包为按本规格**实算生成并自解码回验**的合规字节（CRC-8 按 poly `0x07`/init `0x00`、MSB-first 无反射计算，反射约定属待冻结项），非真机抓包——真实 DUT 链路尚未上线（见 §11 代码现状）。
>
...
- **待冻结清单**（动工前与固件逐项拍板）：字节序、`ver` 终值、CRC-8 参数（含反射约定）、S24 是否紧密 3B、汇顶 bytes 块内布局、`SensorId`/`algorithm_id`/`metric_id` 枚举、GATT UUID、Notify vs Indicate、批是否保证单包、`pktSeq` 重置语义、时钟偏移建立方式——全表见 [归档协议文档 §15](../legacy/BlueTrace_Protocol.md) 与 SPEC §10.4。
...
- 历史全文（含 §15 待冻结清单、§16 评审修订记录）：[`Docs/legacy/BlueTrace_Protocol.md`](../legacy/BlueTrace_Protocol.md)
