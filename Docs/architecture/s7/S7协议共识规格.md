...
>
> **与 BlueTrace v0 协议的区隔**：本文是 **S7 真机现行协议**（0xBB 信封 + Cmd/Key）；[`architecture/BLE协议帧规格_开发者版.md`](../../architecture/BLE协议帧规格_开发者版.md) 是自研 DUT 的 12B 帧头 + protobuf **草案**，两者互不相关，App 侧按通道分别挂解析器。
>
> 深度参考（同目录，随 s7 分支合并已入 main）：[`protocol-b2a.md`](protocol-b2a.md)（B2A 全命令集）、[`protocol-zqdata.md`](protocol-zqdata.md)（zqdata 全量审计与差异核查）。本文为两者之上的**单一开发者共识稿**；ZQDATA 下一代重设计见 [`protocol-zqdata-uhtp-v1.md`](protocol-zqdata-uhtp-v1.md)。
...
- BlueTrace 侧接入架构：分支 `02_parser_registry_design.md`（可注册通道解析器）；自研 DUT 草案（勿混淆）：[`architecture/BLE协议帧规格_开发者版.md`](../../architecture/BLE协议帧规格_开发者版.md)。
