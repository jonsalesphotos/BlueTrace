...
1. **触发器** —— 库把 `gatt.discoverServices()` 的 **`false` 返回值直接丢弃**。原生层拒绝启动发现时，库已把状态置成 `Discovering`，此后回调永不到达，状态**永久停在 Discovering**。这就是我们观测到的"服务发现挂 10s 超时"。
...
## 上游 issue 草稿（待用户过目后提交）

草稿见本文档同目录 [`Nordic重连挂死_issue草稿.md`](Nordic重连挂死_issue草稿.md)。要点：两个 bug 同处三行、给出最小修复建议、附纯 JVM 可复现测试。
