...
| —— | ✅ **库缺陷坐实（07-16 勘误口径）** | 服务发现是全库**唯一**用裸 `lock()`（无 finally）的地方，且**丢弃 `discoverServices()` 的 false 返回值**——两者均为真实代码缺陷（issue #337 缺陷 1/2）。但**本次实测触发不是 false 返回**（分层观测：全部取样挂死 `started=true`），实测丢失发生在返回 true 与原生回调之间（与 MTU 重叠强相关，见终局节）；裸 lock 放大器则与实测完全吻合 |
...
## 尚未闭环：触发器的原生成因（需真机）〔⚠️ 07-16 注：本章按 07-15 的 false-return 假设展开；分层观测后已知实测触发是 "true 后回调丢失"，本章的 false-return 分析降级为**代码缺陷分析**而非本次实测成因——保留原文供追溯〕
