...
- **BLE 后端终局（2026-07-16 拍板）**：自写 `AndroidBleClient` 继续默认；Nordic 保留为可选实验后端；**#24B 转默认无限期暂停**（A/B：自写 4 个连接实例 4/4 收到回调、0 挂死 vs Nordic 7 次请求 3 挂死；实测触发=**`discoverServices()` 返回 true 后原生回调丢失**（与 MTU 重叠强相关），裸 lock 放大为全进程锁死；false-return 丢弃是**独立的库缺陷**（非本次实测触发），上游 issue [#337](https://github.com/nordicsemi/Kotlin-BLE-Library/issues/337) 已提；实验代码封存 `origin/task/25-nordic-reconnect-hang`）。#24A（Kotlin 2.4.10+删逃生阀）已完成合入。
...
- **后续主线 = UWTP v0.2 线**（用户已给完整说明书：协议实验室 + FW-only OTA；固件基线 `apollo4_watch_s7@f480d804`；分支 `feat/uwtp-v02-client` 曾于 07-16 rebase 到当时的 main(`bab7e76`, 280 tests 绿)，其后 main 又前进(身份 MAC/PROFILE_B2A 值/竞态修复)——**开工前需再 rebase 一次**（落差以 `git rev-list --left-right --count main...feat/uwtp-v02-client` 实测为准，不抄本行）；**等用户说"开工"**，真机/串口/装 APK 联调需另行授权）。
