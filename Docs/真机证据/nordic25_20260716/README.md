...
自写侧指标不能从该文件整体直接数出, 必须按下述规则切段并按 gattId 归并后复算(正确口径 = 4 个连接实例 4/4); `hold_nordic.log` 则是单段, Nordic 7/3 可直接复算。
...
- 样本量小(Nordic 7 次请求 / 自写 4 个连接实例、8 行请求), 是**定位性**实验而非统计验收; 50 次量级验收留给将来若重启 #24B 时执行。
