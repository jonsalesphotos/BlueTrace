...
> *iOS 主线原型待补*（X 系列里程碑产出，见 §7.4）。机器可读消息契约见 `Docs/归档/自研协议线_v0/bluetrace_v0.proto`。
>
...
| D-V4-6 | 权限 = 全局硬门：开始采集前一次性门控；**GNSS 在「采集类型」勾选时按需授权**（Q1），否则本次会话不含 GPS 一路；否定「单路缺权限降级」。 |
...
> **机器可读契约（单一事实源，D-10）**：`Docs/归档/自研协议线_v0/bluetrace_v0.proto`。帧头/分片/CRC 链路事实已内联本节 §4.3–4.9（原始来源已归档 `Docs/归档/历史规格_v1-v3/BlueTrace_Protocol.md`）。
> **状态（2026-07-06 口径修正）**：本节 v0.1 "12B 自研信封"方案**整体待重构思，已归档**（`Docs/归档/自研协议线_v0/`）——M7 采集协议改为**在现网 B2A 协议主体上添加/扩展**（B2A/zqdata 真源：`Docs/S7B2A/S7协议共识规格.md`），UWTP（`Docs/UWTP/`）仅作思考方向、可能借用部分设计。新方案成稿前，本节保留为帧层设计素材。
...
- **启用入口（Q1，2026-06-22 用户裁决）**：GNSS **不是设置页全局开关**，而是**每次采集在「采集类型选择」（运行C 下抽屉）里勾选**的独立一路；勾选时**按需授权定位**（拒绝则本次会话不含 GPS 一路、不阻断）。采集中可随时勾选/取消（`SessionController.setGnss` → 起/停 GNSS 订阅 + `gps.csv` 续写）。设置页 GNSS 屏退化为**只读状态/权限**（定位权限 / 系统定位总开关）。
...
- **GNSS 设置项** — 进设置页 GNSS 屏 = **只读状态/权限**（定位权限 / 系统定位总开关 + 去授权/去系统设置）；**启用改为采集类型勾选**（运行C），见 §6.5 Q1。
...
- **真机优先**：启动屏冷/暖启动规则（§5.1）、前台服务保活、权限弹窗与 `BLOCKED`、edge-to-edge、预测返回、MediaStore 导出、存储预检等，在**真机**比模拟器更真实——验收以真机为准，模拟器 / Compose Preview 作快速回归。
- **参考路可现在就上真实 BLE**：标准心率带（HRS `0x180D`）是 SIG 标准、**不依赖自研协议冻结**；若有心率带，参考设备的扫描 / 连接 / 订阅 / 重连 / 保活可用**真实 BLE**提前验证（`BleClient` 接口不变，仅参考路换真实实现）。
- **DUT 仍 Mock**：自研 DUT 走私有 TLV+protobuf，协议未冻结前继续 Mock；冻结后换 Nordic 真实实现（§10.4）。

> 机器契约见 `Docs/归档/自研协议线_v0/bluetrace_v0.proto`。
