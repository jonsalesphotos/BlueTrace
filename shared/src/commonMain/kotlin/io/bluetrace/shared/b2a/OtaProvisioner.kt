 ...
 *
 * 为什么要这层: [B2aOtaSession.provision] 只到 [OtaResult.DoneDownload](下载收讫). 设备在末文件 STOP 后
 ...
 *
 * 读版本 [readVersion] 由外部注入(生产接 [B2aConsole.getDeviceInfo] 的 `swVer`), 使本层与具体读法解耦, Mock 可测.
 * **本层对 [readVersion] 抛异常有容错**([B2aConsole.getDeviceInfo] 超时/解析失败是 *抛* `B2aCommandException`
 * 而非返 null)——[readVersionWithRetry] 捕获非取消异常转 null 重试, 保证 [provisionAndReconnect] 始终返 [OtaResult].
 */
class OtaProvisioner(
    private val session: B2aOtaSession,
    private val ble: BleClient,
    private val device: ScannedDevice,
    /** 读设备软件版本(重连后). 生产 = B2aConsole.getDeviceInfo().swVer; 失败返 null(不抛).  */
    ...
    /**
     * 读版本(重试). 容错 [readVersion] 抛异常(生产 [B2aConsole.getDeviceInfo] 超时/失败会抛 `B2aCommandException`):
