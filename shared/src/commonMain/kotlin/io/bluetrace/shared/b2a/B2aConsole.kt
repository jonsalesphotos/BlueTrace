...
/** 操作日志行(控制台调试面板).  */
data class B2aOpLine(val timeMs: Long, val text: String)

/** 一次命令的失败原因.  */
sealed interface B2aFailure {
    data object Timeout : B2aFailure
    data class DeviceError(val code: Int) : B2aFailure {
        val name: String get() = B2a.errorName[code] ?: "0x${code.toString(16)}"
    }
}

class B2aCommandException(val failure: B2aFailure) :
    Exception(if (failure is B2aFailure.DeviceError) "device error ${failure.name}" else "timeout")

/** 日志拉取进度.  */
data class B2aLogPullProgress(val chunks: Int, val bytes: Int, val done: Boolean)
 ...
 * - **超时**: 默认 3s; 超时抛 [B2aCommandException], 重试交给用户手动触发;
...
class B2aConsole(
    ...
    private val responses = Channel<B2aMessage>(capacity = 16, onBufferOverflow = BufferOverflow.DROP_OLDEST)
    ...
    private val _opLog = MutableSharedFlow<B2aOpLine>(replay = 64, extraBufferCapacity = 64, onBufferOverflow = BufferOverflow.DROP_OLDEST)
    val opLog: SharedFlow<B2aOpLine> = _opLog

    private val _heartbeat = MutableStateFlow<B2aHeartbeat?>(null)
    val heartbeat: StateFlow<B2aHeartbeat?> = _heartbeat
    ...
    fun start() {
        ...
        collectJob = scope.launch {
            ...
            //(B2aFrameDecoder 契约「重连须 reset」; 防跨连接拼出脏多包消息).
            launch {
    ...
    private fun route(msg: B2aMessage) {
        when {
            msg.cmd == B2a.CMD_IND && msg.key == B2a.IND_HEARTBEAT -> {
                B2aHeartbeat.parse(msg.param)?.let { _heartbeat.value = it }
                log("RX heartbeat seq=${_heartbeat.value?.seq} batt=${_heartbeat.value?.batteryPercent}%")
            }
            msg.cmd == B2a.CMD_DEV_CTRL && msg.key == B2a.CTRL_ACK_FILE_LOG && logSink != null -> {
                logSink?.trySend(msg.param)
            }
            // 日志拉取期间设备对 0x07/0x07 回错误 CommAck(BUSY/PARAM…)→ 以异常关闭块通道,
            // 避免被 4s 空闲超时静默吞成「空日志成功」.
            msg.cmd == B2a.CMD_DEV_CTRL && msg.key == B2a.CTRL_FILE_LOG && logSink != null &&
                msg.param.isNotEmpty() && msg.param[0].toInt() != 0 -> {
                val code = msg.param[0].toInt() and 0xFF
                log("RX pull-log NAK ${B2a.errorName[code] ?: code}")
                logSink?.close(B2aCommandException(B2aFailure.DeviceError(code)))
            }
            else -> {
    ...
    private suspend fun request(cmd: Int, key: Int, param: ByteArray = ByteArray(0)): B2aMessage =
        commandMutex.withLock {
            drainStale()
            val frame = B2aFrameCodec.encodeRequest(cmd, key, param)
            ...
            if (reply == null) {
                ...
                throw B2aCommandException(B2aFailure.Timeout)
            }
            reply
        }
    ...
    private suspend fun requestAck(cmd: Int, key: Int, param: ByteArray = ByteArray(0)) {
        ...
        if (!B2a.commAckOk(reply.param)) {
            val code = if (reply.param.isNotEmpty()) reply.param[0].toInt() and 0xFF else 0x01
            throw B2aCommandException(B2aFailure.DeviceError(code))
        }
    }

    // ---- 维护操作 ----

    suspend fun getDateTime(): B2aDateTime =
        B2aDateTime.parse(request(B2a.CMD_GET, B2a.KEY_DATE_TIME).param)
            ?: throw B2aCommandException(B2aFailure.DeviceError(0x03))

    /** 手机当前本地时间 → SET(timezone=0 保持设备时区不改, spec §4.1). 返回同步后的设备时间.  */
    suspend fun syncTime(): B2aDateTime {
     ...
     * week 字段由 y/m/d 自算(固件通常也自算); timezone 见 [B2aDateTime](SET 时 0=保持设备时区).
     * @return SET 后读回的设备时间.
     */
    suspend fun setDateTime(dt: B2aDateTime): B2aDateTime {
        val fixed = dt.copy(week = weekday(dt.year, dt.month, dt.day))
        requestAck(B2a.CMD_SET, B2a.KEY_DATE_TIME, fixed.encode())
        return getDateTime()
    }
    ...
    fun driftSeconds(deviceTime: B2aDateTime): Long {
    ...
    suspend fun getDeviceInfo(): B2aDeviceInfo =
        B2aDeviceInfo.parse(request(B2a.CMD_GET, B2a.KEY_DEV_INFO).param)
            ?: throw B2aCommandException(B2aFailure.DeviceError(0x03))

    suspend fun getSnInfo(): B2aSnInfo =
        B2aSnInfo.parse(request(B2a.CMD_GET, B2a.KEY_SN_INFO).param)
            ?: throw B2aCommandException(B2aFailure.DeviceError(0x03))

    suspend fun getBattery(): B2aBattery =
        B2aBattery.parse(request(B2a.CMD_GET, B2a.KEY_DEV_VOL).param)
            ?: throw B2aCommandException(B2aFailure.DeviceError(0x03))

    /** 功能掩码 u32(逐位含义文档缺失, 仅 hex 展示).  */
    suspend fun getDevFunc(): Long {
        val p = request(B2a.CMD_GET, B2a.KEY_DEV_FUNC).param
        if (p.size < 4) throw B2aCommandException(B2aFailure.DeviceError(0x03))
    ...
    suspend fun getBondState(): Int {
        val p = request(B2a.CMD_GET, B2a.KEY_BOND_STATE).param
        if (p.isEmpty()) throw B2aCommandException(B2aFailure.DeviceError(0x03))
        return p[0].toInt() and 0xFF
    }

    suspend fun getPerson(): B2aPerson =
        B2aPerson.parse(request(B2a.CMD_GET, B2a.KEY_PERSON_DATA).param)
            ?: throw B2aCommandException(B2aFailure.DeviceError(0x03))

    suspend fun setPerson(person: B2aPerson) {
        requestAck(B2a.CMD_SET, B2a.KEY_PERSON_DATA, person.encodeSet())
    }

    /**
     * 逐项读全量快照(B3 下沉自 app 层编排): 单项失败不阻断其余, 失败项为 null,
     * 首个错误记入 [B2aSnapshot.firstError] 供上层提示.
     */
    suspend fun readAll(): B2aSnapshot {
        var firstError: B2aFailure? = null
    ...
    suspend fun findWatch(start: Boolean) {
        requestAck(B2a.CMD_DEV_CTRL, if (start) B2a.CTRL_FIND else B2a.CTRL_FIND_END)
    }
     ...
     */
    suspend fun pullLog(onProgress: (B2aLogPullProgress) -> Unit = {}): ByteArray = commandMutex.withLock {
        ...
        logSink = sink
        try {
            val frame = B2aFrameCodec.encodeRequest(B2a.CMD_DEV_CTRL, B2a.CTRL_FILE_LOG, reqPayload)
            ...
            while (true) {
                ...
                chunks++
                onProgress(B2aLogPullProgress(chunks, assembled.size, done = false))
            }
            log("pull-log done chunks=$chunks bytes=${assembled.size} (idle-timeout heuristic)")
            onProgress(B2aLogPullProgress(chunks, assembled.size, done = true))
    ...
    suspend fun sendPowerCommand(key: Int): Boolean = commandMutex.withLock {
        require(key == B2a.CTRL_POWER_OFF || key == B2a.CTRL_RESET || key == B2a.CTRL_RESTORE) { "not a power command: $key" }
        ...
        val frame = B2aFrameCodec.encodeRequest(B2a.CMD_DEV_CTRL, key, needAck = false)
    ...
    private fun log(text: String) {
        _opLog.tryEmit(B2aOpLine(clock.nowMs(), text))
    }
}

/** [B2aConsole.readAll] 的全量读结果: 读失败的项为 null(上层保留旧值).  */
data class B2aSnapshot(
    val info: B2aDeviceInfo? = null,
    val sn: B2aSnInfo? = null,
    val devFunc: Long? = null,
    val bondState: Int? = null,
    val battery: B2aBattery? = null,
    val deviceTime: B2aDateTime? = null,
    val driftSec: Long? = null,
    val person: B2aPerson? = null,
    val firstError: B2aFailure? = null,
)
