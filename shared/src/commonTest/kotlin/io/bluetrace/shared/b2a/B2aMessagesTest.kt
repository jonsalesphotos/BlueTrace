...
class B2aMessagesTest {

    @Test
    fun dateTime_encodeParseRoundtrip() {
        val dt = B2aDateTime(2026, 7, 2, 14, 5, 9, week = 3, timezone = 8)
        val parsed = B2aDateTime.parse(dt.encode())!!
    ...
    fun dateTime_negativeTimezone_signed() {
        ...
        val b = B2aDateTime(2026, 1, 1, 0, 0, 0, 1, timezone = -5).encode()
        assertEquals(-5, B2aDateTime.parse(b)!!.timezone)
    }

    @Test
    fun dateTime_truncated_returnsNull() {
        assertNull(B2aDateTime.parse(ByteArray(8)))
    }

    @Test
    fun dateTime_invalidFieldValues_returnNull() {
        // RTC 未初始化/恢复出厂后的垃圾值不得进入下游（评审修复 #0：防 monthDays 越界崩溃）
        assertNull(B2aDateTime.parse(ByteArray(9))) // month=0
        assertNull(B2aDateTime.parse(B2aDateTime(2026, 13, 1, 0, 0, 0, 1, 0).encode())) // month=13
        assertNull(B2aDateTime.parse(B2aDateTime(2026, 1, 32, 0, 0, 0, 1, 0).encode())) // day=32
        assertNull(B2aDateTime.parse(B2aDateTime(2026, 1, 1, 24, 0, 0, 1, 0).encode())) // hour=24
    }

    @Test
    fun battery_parse10Bytes() {
        val b = ByteArray(10)
        B2aFrameCodec.writeLe16(b, 0, 280)
        B2aFrameCodec.writeLe16(b, 2, 4130)
        b[4] = 82
        b[5] = 1
        val bat = B2aBattery.parse(b)!!
        ...
        assertNull(B2aBattery.parse(ByteArray(9)))
    }
    ...
    fun snInfo_parse59Bytes_zeroPaddedAsciiAndMac() {
        ...
        assertNull(B2aSnInfo.parse(ByteArray(58)))
    }
    ...
    fun deviceInfo_tlvRoundtrip() {
        val info = B2aDeviceInfo("1.2.7.0", "1.0", "1.0", "23")
        assertEquals(info, B2aDeviceInfo.parse(info.encode()))
    }

    @Test
    fun deviceInfo_truncatedTlv_returnsNull() {
        val bytes = B2aDeviceInfo("1.2.7.0", "1.0", "1.0", "23").encode()
        assertNull(B2aDeviceInfo.parse(bytes.copyOfRange(0, 3))) // sw 段长度域越界
    }

    @Test
    fun person_getSetLayouts() {
        val p = B2aPerson(heightCm = 175, weightKg = 68, gender = 1, birthYear = 1993, birthMonth = 6, birthDay = 12)
        val set = p.encodeSet()
        assertEquals(8, set.size)
        // GET 响应 = SET 前 7 字节布局（spec §4.6）
        assertEquals(p, B2aPerson.parse(set.copyOfRange(0, 7)))
        assertEquals(p, B2aPerson.parse(set)) // 8B 输入同样可解（只读前 7）
        assertNull(B2aPerson.parse(ByteArray(6)))
    }
    ...
    fun heartbeat_parse() {
        ...
        B2aFrameCodec.writeLe16(b, 4, 1234)
        b[6] = 82
        val hb = B2aHeartbeat.parse(b)!!
    ...
    fun commAck_semantics() {
        assertEquals(true, B2a.commAckOk(byteArrayOf(0)))
        assertEquals(false, B2a.commAckOk(byteArrayOf(1)))
        assertEquals(false, B2a.commAckOk(byteArrayOf(5)))
        assertEquals(false, B2a.commAckOk(ByteArray(0)))
    }
