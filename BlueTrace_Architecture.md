...
- 数据落盘按角色独立 CSV；会话级 `session_manifest.json` 记录开始/结束时间、设备 MAC、采样起点 wallclock + monotonic 时间，用于多设备后期对齐（单设备会话该 manifest 仍有用，记录会话边界）。
...
### 12.5 导出到 Downloads（API 24+ 全兼容）

**API 29+ 走 MediaStore**（推荐路径，无需任何存储运行时权限）；
**API 24–28 直接复制到公共 `Environment.DIRECTORY_DOWNLOADS/BlueTrace/` 目录**，前提是已经申请到 `WRITE_EXTERNAL_STORAGE`（参见 §5.1 中 `maxSdkVersion="28"` 的声明 + §5.2 的运行时请求矩阵）。
...
@Serializable data object DeviceAssignment
@Serializable data class Scan(val role: DeviceRole)
@Serializable data object Collect
```
...
`DataCollectionScreen` 展示：按角色分组的连接状态、当前设备、最新解析数据、saved/invalid 计数、当前文件路径、Start/Stop、Send Command、Export to Downloads。
